...
export function OfflineStatusDock() {
  const [state, setState] = useState<DockState>({ auth: null, sync: null, cloud: null });
  ...
  useEffect(() => {
    ...
    async function refresh() {
      try {
        const auth = await invoke<AuthStatus>("desktop_auth_status");
        if (!auth.unlocked) {
          if (!cancelled) setState({ auth, sync: null, cloud: null });
          return;
        }
        ...
        if (!cancelled) setState({ auth, sync, cloud });
      } catch {
  ...
  const view = useMemo(() => {
    ...
    if (!auth) {
      return {
        tone: "starting",
        title: "Waste X Desktop starting",
        detail: "Checking encrypted local state…",
        queued: 0,
      };
    }
    if (!auth.unlocked) {
    ...
    if (review > 0) {
      return {
        tone: "problem",
        title: "Sync review required",
        detail: `${review} item${review === 1 ? "" : "s"} need review. Later physical work remains safely queued.`,
        queued,
      };
    }
    if (sync?.authRequired) {
      return {
        ...
        detail: "Local operations remain available, but upload is paused until an online sign-in renews this device session.",
        queued,
      };
    }
    if (bootstrapAge !== null && bootstrapAge >= 14) {
      return {
        tone: "problem",
        title: "Offline working set is stale",
        detail: `Last Cloud bootstrap was ${bootstrapAge} days ago. Existing local work is preserved; reconnect before relying on new reference data.`,
        queued,
      };
    }
    if (sync?.running) {
      return {
        tone: "syncing",
        title: "Syncing with Waste X Cloud",
        detail: queued > 0 ? `${queued} local change${queued === 1 ? "" : "s"} waiting for acknowledgement.` : "Checking for Cloud changes…",
        queued,
      };
    }
    if (sync?.cloudReachable) {
      return {
        ...
        detail: queued > 0
          ? `${queued} local change${queued === 1 ? "" : "s"} will upload automatically in device order.`
          : `Last successful sync ${relativeTime(sync.lastSuccessAt)}.`,
        queued,
      };
    }

    if (bootstrapAge !== null && bootstrapAge >= 7) {
      return {
        tone: "warning",
        title: "Working offline · cache aging",
        detail: `Local operations are safe. Working set was last refreshed ${bootstrapAge} days ago; ${auth.offlineDaysRemaining} offline-auth day${auth.offlineDaysRemaining === 1 ? "" : "s"} remain.`,
        queued,
      };
    }
    ...
    return {
      tone: "offline",
      title: "Working safely offline",
      detail: `${queued} change${queued === 1 ? "" : "s"} stored locally · ${auth.offlineDaysRemaining} offline-auth day${auth.offlineDaysRemaining === 1 ? "" : "s"} remaining.`,
      queued,
    };
  }, [state]);
