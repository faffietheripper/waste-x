...
const TICKET_AUTHORITY: &str = "MANAGEMENT_SITE";
const PDF_TEMPLATE_VERSION: i64 = 1;
...
fn ticket_block_reason(load: &TicketLoad) -> Option<String> {
    if matches!(load.status.as_str(), "completed" | "rejected" | "cancelled") {
        return Some("This load is already closed. The canonical site ticket must be issued before site completion.".to_string());
    }

    match field_workflow_step(&load.payload).as_deref() {
        Some("DELIVERED") => {}
        Some(step) => {
            return Some(format!(
                "Driver delivery confirmation required. Current Mobile field step: {step}."
            ));
        }
        None => {
            return Some(
                "Driver delivery confirmation has not reached this Desktop yet. Sync or relay the Mobile FIELD_DELIVERED event before issuing the site ticket."
                    .to_string(),
            );
        }
    }
    ...
    if load.direction == "incoming" && load.status != "accepted" {
        return Some(
            "The management site must accept this incoming load before issuing the ticket."
                .to_string(),
        );
    }
    ...
    if !net.is_finite() || net <= 0.0 {
        return Some("A positive final net quantity is required before ticket issue.".to_string());
    }
    ...
    if value_string(&load.payload, "wasteDescriptionSnapshot")
        ...
        .is_empty()
    {
        return Some("Confirm the actual waste description before ticket issue.".to_string());
    }

    if value_string(&load.payload, "driverId").is_none() {
        return Some("Driver identity is required before ticket issue.".to_string());
    }
    if value_string(&load.payload, "vehicleId").is_none() {
        return Some("Vehicle identity is required before ticket issue.".to_string());
    }
...
fn derive_ticket_number(load: &TicketLoad) -> String {
    ...
    format!(
        "{}-L{:02}-{}",
        normalise_job_number(&load.job_number),
        load.load_number,
        load_code(&load.id)
    )
}
...
fn build_ticket_pdf(load: &TicketLoad, ticket_number: &str, issued_at: &str) -> Vec<u8> {
    ...
    content.push_str(&pdf_text_line(48, 765, 10, true, "MANAGEMENT-SITE DIGITAL WASTE TICKET"));
    ...
    content.push_str(&pdf_text_line(48, 680, 9, true, "DELIVERY / ISSUE"));
    content.push_str(&pdf_text_line(48, 663, 10, false, &format!("Driver field state: {}", field_workflow_step(&load.payload).unwrap_or_else(|| "Not recorded".to_string()))));
    content.push_str(&pdf_text_line(48, 647, 10, false, &format!("Issued by management site: {issued_at}")));
    content.push_str(&pdf_text_line(48, 617, 9, true, "WASTE"));
    content.push_str(&pdf_text_line(48, 600, 10, false, &format!("EWC: {ewc}")));
    content.push_str(&pdf_text_line(48, 584, 10, false, &format!("Description: {waste}")));
    content.push_str(&pdf_text_line(48, 554, 9, true, "WEIGHTS"));
    content.push_str(&pdf_text_line(48, 537, 10, false, &format!("Gross: {gross} {metric}")));
    content.push_str(&pdf_text_line(48, 521, 10, false, &format!("Tare: {tare} {metric}")));
    content.push_str(&pdf_text_line(48, 505, 11, true, &format!("Net: {net} {metric}")));
    content.push_str(&pdf_text_line(48, 475, 9, true, "TRANSPORT"));
    content.push_str(&pdf_text_line(48, 458, 10, false, &format!("Driver ID: {driver}")));
    content.push_str(&pdf_text_line(48, 442, 10, false, &format!("Vehicle ID: {vehicle}")));
    ...
    content.push_str(&pdf_text_line(48, 78, 8, false, "Reprints must use these exact original PDF bytes."));
...
fn existing_ticket_state(
    connection: &Connection,
    load: &TicketLoad,
) -> Result<Option<DesktopTicketState>, String> {
    let row = connection
        .query_row(
...
fn ticket_state(connection: &Connection, load: &TicketLoad) -> Result<DesktopTicketState, String> {
    if let Some(existing) = existing_ticket_state(connection, load)? {
        return Ok(existing);
    }
...
pub fn desktop_issue_ticket(
    ...
    if let Some(reason) = ticket_block_reason(&load) {
        return Err(reason);
    }
    ...
    sequence = next_device_sequence(&transaction)?;
    ...
    transaction
        .execute(
            "INSERT INTO local_sync_queue (
             ...
             ) VALUES (?1, ?2, ?3, ?4, ?5, 'job_load', ?6, 'LOAD_DETAILS_UPDATED', ?7, ?8, ?9, ?10, ?11, ?11, 'PENDING', 0, ?11)",
            params![
                event_id,
                actor.organisation_id,
                site_id,
    ...
    transaction
        .execute(
            "INSERT INTO local_audit_event (
             ...
             ) VALUES (?1, ?2, 'TICKET_ISSUED', 'job_load', ?3, ?4, ?5)",
            params![event_id, actor.user_id, load.id, serde_json::to_string(&snapshot).map_err(|e| e.to_string())?, now],
        )
        .map_err(|e| e.to_string())?;
