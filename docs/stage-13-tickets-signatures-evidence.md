...
## 13.1 — Offline ticket identity — implemented foundation

Mobile SQLCipher schema introduces `local_ticket`.
...
## 13.2 — Offline PDF — implemented generation/storage foundation

Mobile SQLCipher schema version 7 adds `local_ticket_document`.

Implemented:

- render from the immutable local ticket snapshot only
- generate PDF bytes with no Cloud/API dependency
- store the exact PDF bytes as an encrypted SQLCipher BLOB
- SHA-256 the final PDF bytes before storage
- retain byte length, generation timestamp and template version
- idempotent generation: once a PDF exists for the issued ticket, repeated generation requests return the same stored document instead of silently creating a different legal document
- expose PDF generation, size, template version and full SHA-256 in the Mobile ticket UI

Still to certify/complete for 13.2:

- run Mobile TypeScript/native smoke testing with `pdf-lib` on the Stage 13 branch
- add a user-facing PDF viewer/export surface while preserving encrypted-at-rest storage
- prove generated bytes survive a full app restart while Cloud is unavailable
- later attach the generated PDF into the common evidence/upload lifecycle rather than creating a second document identity

The first template intentionally uses built-in PDF fonts and normalises unsupported characters. A later template pass can embed a production font when the ticket layout and legal content are finalised.
